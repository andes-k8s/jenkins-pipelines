...
def call(body) {
  ...
  sh "ls -lah"
  sh "docker run --rm -v \$(pwd)/${playbookFileName}:/ansible/${playbookFileName} -v \$(pwd)/${hostsFileName}:/app/${hostsFileName} -v \$(pwd)/${pubKeyFileName}:/root/.ssh/id_rsa --entrypoint=ls ${ansibleImage} -i ${hostsFileName} 
  sh "docker run --rm -v \$(pwd)/${playbookFileName}:/ansible/${playbookFileName} -v \$(pwd)/${hostsFileName}:/app/${hostsFileName} -v \$(pwd)/${pubKeyFileName}:/root/.ssh/id_rsa --entrypoint=pwd ${ansibleImage} -i ${hostsFileName} 
